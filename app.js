...
function setupShaders() {
  ...
  pwgl.uniformSpotDirectionLoc = gl.getUniformLocation(
    shaderProgram,
    "uSpotDirection"
  ); // spot light를 퐁 반사 모델로 구현할 때 필요한 유니폼 변수 -> 스포트 광원이 방출되는 방향 벡터 (= 원뿔 축)
 ...
 *
 *
 * 1. 일단 spotDirection(원뿔의 축 방향 벡터. 즉, 스포트 광원이 방출되는 방향) 값을
 * 자바스크립트에서 지정한 뒤 프래그먼트 셰이더의 유니폼 변수에 쏴줘야 하므로,
 * 해당 유니폼 변수의 WebGLUniformLocation 객체를 전역변수에 지정해놓음.
 *
 * 또한, setupLights() 함수에서 uSpotDirection 유니폼 변수로 미리 지정된 벡터값을
 * 쏴주는 코드를 추가해 줌.
 *
 * 이 때, 각 유니폼 변수에 전달해 줄 값들을 원본 코드대로 수정해줄 것!
 *
 *
 * 2. 이번 예제에서는 floor(바닥)에만 스포트 광원을 비춰줄 것이므로, 큐브나 테이블을 안그려도 됨.
 * 따라서 큐브를 그릴 때 필요한 WebGLBuffer들을 설정하는 setupCubeBuffers() 함수 제거해도 됨.
 *
 *
 * 3. 2번과 동일한 이유로 큐브를 그리는 함수와 테이블 그리는 함수인
 * drawCube(), drawTable() 을 제거함.
 *
 *
 * 4. 테이블 및 큐브를 그려줄 필요가 없고, 바닥만 그려주면 되기 때문에,
 * 모델뷰행렬을 초기화하지 않아도 됨.
 *
 * 따라서, 모델뷰행렬의 스택을 관리하는 두 함수
 * pushModelViewMatrix(), popModelViewMatrix() 도 제거해 줌.
 *
 *
 * 5. 이 예제에서는 애니메이션을 계산할 필요가 없기 때문에,
 * draw() 함수 내에서 애니메이션 관련 코드를 제거해 줌.
 *
 *
 * 6. 5번에 의해 애니메이션을 그릴 필요가 없으니 FPS 카운터도 필요없음.
 * 따라서 관련 전역변수인 pwgl.nbrOfFramesForFPS 도 필요없음.
 *
 *
 * 7. draw() 함수 내에서 '뷰 변환'을 모델뷰행렬에 적용해줄 때,
 * 카메라의 위치를 원본 코드에 맞게 수정해줄 것
 */
